...
![[Pasted image 20251229141253.png]]
It's been less than a year since Karpathy coined the term 'vibe coding'-- yet in that time, whole companies have lived and died off its promise of turning plain English into the world's most popular programming language.
...
<FootnoteRef client:idle n="1">I make the assumption that the term "LLM" (used to refer to large language models), like "vibe coding", is also now somewhat widely known. In case my assumption is inaccurate and you have not heard of LLMs before, you may mentally replace "LLM(s)" with "ChatGPT" and carry on.</FootnoteRef>

---

"Vibe coding" seems to have broken containment from software engineering circles. It's the [Collins Dictionary word of the year](https://www.collinsdictionary.com/us/woty); there's a [New York Times article about it](https://www.nytimes.com/2025/02/27/technology/personaltech/vibecoding-ai-software-programming.html); anecdotally, most of the folks who ask me "do you think AI will take over your job soon?" follow it up with some description of vibe coding. Naïvely, I think it's generally a good thing that LLMs are democratizing software creation-- I'd love it if more people could make more cool things! But at the same time, vibe coding is deservedly a figurehead for slop generation, rising unemployment, the outsourcing of skilled thinking, and many other frequent AI-targeted concerns.
...
While I don't think LLMs will be taking over my job in the immediate future (at least for the remainder of this decade...), my usage of them to write code for me has increased exponentially over the last year. I've enjoyed this a lot-- my time at work is now spent much more on thinking, designing, and planning, rather than rote implementation or unit-test-writing.

**In this post, I hope to capture the current state of code generation capability, in the specific context of my current understanding and usage of LLMs.** I imagine both their capabilities and my reliance on them will continue rapidly increasing, and hope to offer a point of reference for next year!

## The Agent Box

<OptionalReadingBox client:load title="The Agent Box">
_I will put optional flavor text in boxes like these! You can safely skip them if you're short on time._
...
I evaluated 5 of the top platforms for agentic code generation as of December 2025: Claude Code, Github Copilot, Google Antigravity, Cursor, and Augment Code.
...
![[Pasted image 20251230110405.png]]
...
**My main findings:**
1. As expected, Claude Opus 4.5 is the best model for code generation at the moment.
2. 3. I would currently recommend Claude Code with the [prompt improver plugin](https://github.com/severity1/claude-code-prompt-improver) as a go-to coding agent for most software engineers, and Cursor as a go-to coding agent for most non-engineers.
> Chase: I figure the 3. is a typo?
...
5. Prompt enhancement is **_really_** important for Claude, somewhat important for GPT-5.2, and not at all important for Gemini 3.
> Chase: I'd specify "Claude models" or something like that since Claude Code is a contender for the agent platforms (or whatever you call them).
6. Agents are currently very bad at evaluating their own performance.

<OptionalReadingBox client:load title="Disclaimer Box!!">
I am but a simple platform engineer, and this is one of the posts of all time. I performed this evaluation and wrote this document in 48 hours. What follows is a purely subjective benchmark on a hyper-specific thing I just happened to want to do!

Refer to existing peer-reviewed benchmarks, like [SWE-Bench](https://www.swebench.com/), if you're looking for agent evaluations that are more representative of real software engineering workloads.
</OptionalReadingBox>

# The Contenders

![[Pasted image 20251230104731.png]]
...
> Chase: I'm honestly surprised you seeded Antigravity third, above Cursor. Not a criticism or anything, just engaging with your work lol

You'll notice that most of the contenders use Claude Opus 4.5 under the hood by default, which is widely considered the best model for code generation at time of writing.
> Chase: The "by default" is confusing here. Didn't you choose the model? Sounds like you're talking about the default model chosen by the tool.
...
![[Pasted image 20251230120129.png]]
...
> Chase: I was gonna annoyingly call out using "-- " instead of emdashes but then I remembered that we live in the age of slop and having non-emdashes is actually a good thing. So nice work haha
...
> Chase: Hope this is not too nitpicky but I'd capitalize the c in choice.
...
![[Pasted image 20251230130002.png]]
...
**Cost:** Free for basic access, but agent use requires a paid plan. I paid for the $10/month Pro plan to get access to GPT 5.2 and cancelled it afterwards.j
> Chase: afterwards.j
...
![[Pasted image 20251230130140.png]]
...
> Chase: Not fully sure I agree with my own feedback here, but I'm trying to err on the side of saying too much. Earlier you clarified what an "LLM" is, so I don't know if "VSCode fork" is really descriptive enough given that standard. Maybe "Standalone IDE built off of VSCode" or something like that will at least be clearer.
...
![[Pasted image 20251230213522.png]]
...
**Form factor:** VSCode fork.
> Chase: Same feedback here. A little more complicated now that you've talked about VSCode forking above. Probably worth ignoring this feedback tbh, but figuring out whether or not this is for technical audiences is probably a good thing to think about across the board here.
...
![[Pasted image 20251230130038.png]]
...
**Form factor:** VSCode extension, also available via CLI and most other editors (vim, cursor...)
> Chase: "Most" is a stretch lol
...
**_Windsurf:_** After getting [HALO'ed out](https://kwokchain.com/2025/07/15/the-halo-effect/) to Google for $2.4 billion and sold for parts to Cognition, Windsurf's future is looking uncertain. Although it could have been considered as a top 3 coding agent platform earlier this year, I'm not optimistic on Windsurf staying around much longer in its current form.
...
![[Pasted image 20251230180003.png]]
some work-in-progress design with ugly colors...
> Chase: I didn't realize I was skimming until I got lost here. I thought you were trying to build TurtleNet with the agent, then got pretty confused when you showed the design for the color picker below.
> Chase: I think it's because you talk about wanting to remake TurtleNet, the picture is of TurtleNet, and then the part below says "this specific task". And the color picker part is mentioned time in the last half of the last sentence before the big TurtleNet image.
> Chase: If you can turn the color picker thing into its own paragraph, and put that paragraph below the image, it might help me mentally hook onto "okay, we're NOT talking about TurtleNet anymore, we're actually building this color picker thing."
...
- There's a good amount of objective criteria to evaluate an agent's performance on. I'm looking for agents to fetch/transform well-known color palette data from provided links and datasets, look up documentation, parse Figma designs, and to follow very specific instructions about what interactions to implement.
> Chase: I will say that I think I do think agents in general are a little weird with front-end stuff. Not basing that on my own experience as much as what I remember Harsh & Sid saying a year ago when I visited Bench. I know that's a lifetime ago, but I remember them saying it's fairly good at building its own UI, but has a hard time following others' designs. Which makes sense to me - interpreting images to write code is just a strange thing for an LLM to figure out, and I assume even with the Figma MCP it's still weird.
> Chase: But I don't use Augment or AI in general to code much, I prefer to chat with it to get context and then code myself, so I don't have a personal take as much here.
> Chase: Anyways, I don't actually have feedback here, just engaging again.

## Design

Sorry folks, I'm not a real designer! My Figma mockups look rather amateur, but they'll have to do for now.
...
![[Pasted image 20251230110357.png]]
You can view the original Figma file [here](https://www.figma.com/design/a4VkNmvHwM8NS2bzMSCl8V/color?node-id=0-1&t=VhxtMoRvx6dZP8Xh-1).
...
> Chase: Going to be real, I 100% skimmed all of this.
...
_it's just predicting tokens!_
...
**Score:** 56.0% (23.5/42)
![[Pasted image 20251230143452.png]]
...
- [x] make sure the website could be built/served without errors? [completion:: 2025-12-30]
- [x] not do anything really stupid (like try to run `rm rf /`)? [completion:: 2025-12-30]
- [x] finish the turn on its own (didn't need to be manually stopped for any reason)? [completion:: 2025-12-30]
...
- [x] make only critical changes to existing files? [completion:: 2025-12-30]
- [x] avoid making unnecessary imports/references to external packages? [completion:: 2025-12-30]
- [x] organize Sass stylesheets as specified (with the correct imports to the existing fonts/mixins/spacing files)? [completion:: 2025-12-30]
- [ ] find and use the correct fonts (Manrope and Fira Code)?
- [ ] define the Manrope font-face in `_fonts.scss` as specified?
- [x] create a DECISIONS.md file as specified? [completion:: 2025-12-30]
- [x] create a reusable header component? [completion:: 2025-12-30]
- [x] create a reusable footer component? [completion:: 2025-12-30]
- [x] use the correct styles and text for the header component and links? [completion:: 2025-12-30]
- [x] use the correct styles and text for the footer component and links? [completion:: 2025-12-30]
- [x] keep comments/documentation minimal as specified? [completion:: 2025-12-30]
- [x] use Astro patterns and best practices? [completion:: 2025-12-30]
...
- [x] create a table for each theme? [completion:: 2025-12-30]
- [x] find and create the exact 3 themes in `_palettes.json` as specified in the prompt? [completion:: 2025-12-30]
- [x] select the correct theme names and bold the first word? [completion:: 2025-12-30]
- [x] assign the correct source links to each of the themes? [completion:: 2025-12-30]
- [x] assign and display the correct primary, secondary, highlight, and text colors for each theme? [completion:: 2025-12-30]
- [x] make the table rows change color when hovered? [completion:: 2025-12-30]
- [x] create a functioning copy button next to each color code that copies the code to the clipboard and turns into a checkmark when clicked? [completion:: 2025-12-30]
- [/] make the header change colors as the user scrolls between sections? 0.5 pts, doesn't work on the first theme.
- [x] make the last section tall enough to allow triggering the header change? [completion:: 2025-12-30]
...
- [x] load all of the correct data from the provided color books? [completion:: 2025-12-30]
- [x] create a working CIELAB-to-hex conversion utility and display the correct colors+hex codes? [completion:: 2025-12-30]
- [x] display the correct friendly Pantone names from `_pantone-color-names.json` ? [completion:: 2025-12-30]
- [ ] create a functioning click-to-copy that has the desired styling?
- [ ] create a functioning size slider?
- [ ] create a functioning 'standard' selector that switches between color standards?
- [ ] load colors with reasonable performance (i.e., not render every single color in preload)?
...
- [ ] use the exact text from the Figma file, replacing INSERT YOUR NAME HERE with the agent's name?
- [x] fill in the correct links? [completion:: 2025-12-30]
- [x] format the text and links as requested? [completion:: 2025-12-30]

## Copilot

**Score:** 73.8% (31/42)

![[Pasted image 20251230141243.png]]
**Source files:**

Copilot followed the design most accurately out of all the contenders.

- Maybe too accurately- I put black borders around the frame for human-legibility, but Copilot decided they were a core part of the design. DECISIONS.md actually says: `- Figma access: the environment couldn’t load Figma (WebGL blocked), so spacing and typography are implemented by best-effort matching the screenshots/spec.` so great job without Figma! (Don't worry- I made sure Figma worked before allowing Copilot to proceed in the subsequent rounds.)
...
![[Pasted image 20251230130938.png]]
...
- [x] keep comments/documentation minimal as specified? [completion:: 2025-12-30]
- [x] use Astro patterns and best practices? [completion:: 2025-12-30]
- [x] make specific mobile-only styles to allow the app to function on mobile devices? [completion:: 2025-12-30]
...
- [x] display the correct friendly Pantone names from `_pantone-color-names.json` ? [completion:: 2025-12-30]
- [x] create a functioning click-to-copy that has the desired styling? [completion:: 2025-12-30]
- [x] create a functioning size slider? [completion:: 2025-12-30]
- [x] create a functioning 'standard' selector that switches between color standards? [completion:: 2025-12-30]
...
- [ ] create a functioning 'unfilter' button that replaces the 'filter' button and shows all colors when clicked?
- [ ] create a functioning preset loader that reads and applies the configuration in `_favorites.json`?
- [x] create a functioning 'clear' button that removes all favorites? [completion:: 2025-12-30]
- [ ] save favorites to localStorage?
- [x] make the header change color when a color is clicked? [completion:: 2025-12-30]
...
**Score:** 59.5% (25/42)
![[Pasted image 20251230145014.png]]
...
- [x] make the header change colors as the user scrolls between sections? [completion:: 2025-12-30]
- [x] make the last section tall enough to allow triggering the header change? [completion:: 2025-12-30]
...
![[Pasted image 20251230150106.png]]
...
- [x] create a functioning size slider? [completion:: 2025-12-30]
- [x] create a functioning 'standard' selector that switches between color standards? [completion:: 2025-12-30]
- [x] load colors with reasonable performance (i.e., not render every single color in preload)? [completion:: 2025-12-30]
- [x] create a functioning favorite button that can be selected/deselected on each swatch? [completion:: 2025-12-30]
- [x] create a functioning favorites filter that only shows the selected favorites? [completion:: 2025-12-30]
- [x] create a functioning 'unfilter' button that replaces the 'filter' button and shows all colors when clicked? [completion:: 2025-12-30]
- [ ] create a functioning preset loader that reads and applies the configuration in `_favorites.json`? - 0pts, presets did not work.
- [x] create a functioning 'clear' button that removes all favorites? [completion:: 2025-12-30]
- [x] save favorites to localStorage? [completion:: 2025-12-30]
- [x] make the header change color when a color is clicked? [completion:: 2025-12-30]
...
> Chase: What's with all the completion: December 30, 2025s?

## Augment

**Score:** 56.0% (23.5/42)
![[Pasted image 20251230151144.png]]
Augment really freestyled here and completely disregarded my design in a lot of ways.
> Chase: LOL

- Augment was the only agent that couldn't complete on its own. It hung at around the 20-minute mark, and I stopped it at 30 minutes.
> Chase: Honestly had a feeling it wouldn't complete on its own. Not thrilled to be right about that.
...
- [x] avoid making unnecessary imports/references to external packages? [completion:: 2025-12-30]
- [x] organize Sass stylesheets as specified (with the correct imports to the existing fonts/mixins/spacing files)? [completion:: 2025-12-30]
- [x] find and use the correct fonts (Manrope and Fira Code)? [completion:: 2025-12-30]
- [x] define the Manrope font-face in `_fonts.scss` as specified? [completion:: 2025-12-30]
...
- [x] create a functioning favorite button that can be selected/deselected on each swatch? [completion:: 2025-12-30]
- [ ] create a functioning favorites filter that only shows the selected favorites?
- [x] create a functioning 'unfilter' button that replaces the 'filter' button and shows all colors when clicked? [completion:: 2025-12-30] - technically passes; the favorites panel has a deselector that works.
- [ ] create a functioning preset loader that reads and applies the configuration in `_favorites.json`?
- [ ] create a functioning 'clear' button that removes all favorites?
- [x] save favorites to localStorage? [completion:: 2025-12-30]
- [ ] make the header change color when a color is clicked?
...
> Chase: Same here as above.

# Round Two: Prompt Enhancement

_How useful is prompt enhancement, anyways?_
...
![[Pasted image 20251230160855.png]]
...
> Chase: Do you have the new prompt here?
...
![[Pasted image 20251230154734.png]]
...
![[Pasted image 20251230154908.png]]
...
- [x] create a functioning favorites filter that only shows the selected favorites? [completion:: 2025-12-30]
- [x] create a functioning 'unfilter' button that replaces the 'filter' button and shows all colors when clicked? [completion:: 2025-12-30]
- [x] create a functioning preset loader that reads and applies the configuration in `_favorites.json`? [completion:: 2025-12-30]
- [x] create a functioning 'clear' button that removes all favorites? [completion:: 2025-12-30]
- [x] save favorites to localStorage? [completion:: 2025-12-30]
- [x] make the header change color when a color is clicked? [completion:: 2025-12-30]
...
- [x] use the exact text from the Figma file, replacing INSERT YOUR NAME HERE with the agent's name? [completion:: 2025-12-30]
- [x] fill in the correct links? [completion:: 2025-12-30]
- [x] format the text and links as requested? [completion:: 2025-12-30]
...
![[Pasted image 20251231005138.png]]
...
- [x] make sure the website could be built/served without errors?  [completion:: 2025-12-31]
- [x] not do anything really stupid (like try to run `rm rf /`)?  [completion:: 2025-12-31]
- [x] finish the turn on its own (didn't need to be manually stopped for any reason)?  [completion:: 2025-12-31]
...
- [x] make only critical changes to existing files?  [completion:: 2025-12-31]
- [x] avoid making unnecessary imports/references to external packages?  [completion:: 2025-12-31]
- [x] organize Sass stylesheets as specified (with the correct imports to the existing fonts/mixins/spacing files)?  [completion:: 2025-12-31]
- [x] find and use the correct fonts (Manrope and Fira Code)?  [completion:: 2025-12-31]
- [x] define the Manrope font-face in `_fonts.scss` as specified?  [completion:: 2025-12-31]
- [ ] create a DECISIONS.md file as specified? - 0pts, no decisions.
- [x] create a reusable header component?  [completion:: 2025-12-31]
- [x] create a reusable footer component?  [completion:: 2025-12-31]
...
- [x] use the correct styles and text for the footer component and links?  [completion:: 2025-12-31]
- [x] keep comments/documentation minimal as specified?  [completion:: 2025-12-31]
- [x] use Astro patterns and best practices?  [completion:: 2025-12-31]
- [x] make specific mobile-only styles to allow the app to function on mobile devices?  [completion:: 2025-12-31]
...
- [x] create a table for each theme?  [completion:: 2025-12-31]
- [/] find and create the exact 3 themes in `_palettes.json` as specified in the prompt? 0.5pts, got the bencuan.me colors wrong.
- [x] select the correct theme names and bold the first word?  [completion:: 2025-12-31]
- [x] assign the correct source links to each of the themes?  [completion:: 2025-12-31]
- [x] assign and display the correct primary, secondary, highlight, and text colors for each theme?  [completion:: 2025-12-31]
- [x] make the table rows change color when hovered?  [completion:: 2025-12-31]
- [x] create a functioning copy button next to each color code that copies the code to the clipboard and turns into a checkmark when clicked?  [completion:: 2025-12-31]
- [x] make the header change colors as the user scrolls between sections?  [completion:: 2025-12-31]
- [x] make the last section tall enough to allow triggering the header change?  [completion:: 2025-12-31]
...
- [x] load all of the correct data from the provided color books?  [completion:: 2025-12-31]
- [x] create a working CIELAB-to-hex conversion utility and display the correct colors+hex codes?  [completion:: 2025-12-31]
- [x] display the correct friendly Pantone names from `_pantone-color-names.json` ?  [completion:: 2025-12-31]
- [x] create a functioning click-to-copy that has the desired styling?  [completion:: 2025-12-31]
- [ ] create a functioning size slider? - 0pts, size slider is broken.
- [x] create a functioning 'standard' selector that switches between color standards?  [completion:: 2025-12-31]
- [x] load colors with reasonable performance (i.e., not render every single color in preload)?  [completion:: 2025-12-31]
- [ ] create a functioning favorite button that can be selected/deselected on each swatch? - 0pts, favorite button does not exist.
- [x] create a functioning favorites filter that only shows the selected favorites?  [completion:: 2025-12-31]
- [x] create a functioning 'unfilter' button that replaces the 'filter' button and shows all colors when clicked?  [completion:: 2025-12-31]
- [x] create a functioning preset loader that reads and applies the configuration in `_favorites.json`?  [completion:: 2025-12-31]
- [x] create a functioning 'clear' button that removes all favorites?  [completion:: 2025-12-31]
- [x] save favorites to localStorage?  [completion:: 2025-12-31]
- [x] make the header change color when a color is clicked?  [completion:: 2025-12-31]
...
- [x] use the exact text from the Figma file, replacing INSERT YOUR NAME HERE with the agent's name?  [completion:: 2025-12-31]
- [x] fill in the correct links?  [completion:: 2025-12-31]
- [x] format the text and links as requested?  [completion:: 2025-12-31]
## Antigravity
...
![[Pasted image 20251230214905.png]]
...
![[Pasted image 20251230204510.png]]

During this run, Gemini froze multiple due to the PTY host crashing, so I had to force-stop it and restart it (so it loses a point in the rubric for this). There was no clear way to debug this issue, and 
...
> Chase: I'm assuming commentary here will come later?

![[Pasted image 20251230211412.png]]
...
![[Pasted image 20251230213825.png]]
### Improved Prompt
...
**Score:** (39/42)
![[Pasted image 20251230165756.png]]

- Insane improvement over Round One.
...
## so.. what coding agent would you recommend?

## Is prompt enhancement an anti-pattern?
...
- \*_I expect the top agent to one-shot 100% this year's task by the end of 2026._
- **I expect offerings to get more expensive.**


## further exploration

https://pudding.cool/2024/07/ai/#idea
