    ...
    return (
        <footer>
            ...
            <div className="footer-icons">
                {icons.map((val) =>
                    (<a href={val.link} key={val.name} className="footer-icon" target="_blank" rel={(val.me) ? "me noreferrer" : "noreferrer"} aria-label={val.name}><FontAwesomeIcon icon={val.icon} /></a>)
                )}
            </div>


            {showArchives && <><span className="close-archives" onClick={() => setShowArchives(false)}>x</span> /</>}
            {
                showArchives ?
                    ([2018, 2019, 2020, 2021, 2022].map((val => (<React.Fragment key={val} > <a className="link" href={`/${val}`}>{val}</a>&nbsp;/&nbsp;</React.Fragment >))))
                    : (<><span tabIndex={0} role="button" className="link show-archive" onClick={() => { setShowArchives(true) }}>archives</span>&nbsp;//&nbsp;</>)
            }
